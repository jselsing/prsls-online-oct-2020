...
Fortunately, for Node.js functions there is the [middy](https://github.com/middyjs) middleware engine. It comes with a [SSM middleware](https://github.com/middyjs/middy/blob/master/docs/middlewares.md#ssm) that can implement the flow for us:
...
6. Repeat step 3-5 to create another `/{service-name}/dev/search-restaurant/config` parameter, also set it to
...
* `cacheExpiryInMillis: 5 * 60 * 1000` allows the cached value to be expired after 5 minutes. So if we change the configuration in SSM Parameter Store, then the concurrent executions would load the new value when their cache expires, without needing a deployment.
...
6. Reload the index page, since we set the cache expiry to 5 minutes, you might have to reload a few times before you see the index page return 4 restaurants instead.
